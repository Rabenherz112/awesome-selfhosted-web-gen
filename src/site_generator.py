...
class SiteGenerator:
    ...
    def _generate_app_detail_pages(self, applications: List[Application]):
        ...
        if self.related_apps_config.get('scoring', {}).get('semantic_similarity', {}).get('enabled', True):
            print("Semantic similarity scoring is enabled. The generation of the app detail pages will take longer...")
    ...
    def _find_related_apps(self, target_app: Application, all_applications: List[Application]) -> List[Application]:
        ...
        for app in all_applications:
            ...
            if scoring_config.get('semantic_similarity', {}).get('enabled', True):
                semantic_score = self._calculate_semantic_similarity(target_app, app, all_applications)
            ...
            if scoring_config.get('categories', {}).get('enabled', True):
                ...
                category_score = len(common_categories) * scoring_config.get('categories', {}).get('points_per_match', 4)
            ...
            if scoring_config.get('alternatives', {}).get('enabled', True):
                alt_score = self._calculate_alternative_similarity(target_app, app, scoring_config)
                score += alt_score
                score_breakdown['alternatives'] = alt_score

            # 4. Fork relationships
            if scoring_config.get('forks', {}).get('enabled', True):
                fork_score = self._calculate_fork_similarity(target_app, app, scoring_config)
                score += fork_score
                score_breakdown['forks'] = fork_score

            # 5. Same programming language
            if scoring_config.get('language', {}).get('enabled', True):
                if app.language and target_app.language and app.language.lower() == target_app.language.lower():
                    language_score = scoring_config.get('language', {}).get('same_language_score', 3)
                    score += language_score
                    score_breakdown['language'] = language_score

            # 6. Similar platform support
            if scoring_config.get('platforms', {}).get('enabled', True):
                common_platforms = set(app.platforms or []) & set(target_app.platforms or [])
                platform_score = len(common_platforms) * scoring_config.get('platforms', {}).get('points_per_match', 2)
                score += platform_score
                score_breakdown['platforms'] = platform_score

            # 7. Same license type (free vs non-free)
            if scoring_config.get('license', {}).get('enabled', True):
                if app.license and target_app.license:
                    ...
                    if target_is_nonfree == app_is_nonfree:
                        license_score = scoring_config.get('license', {}).get('same_type_score', 2)
                        score += license_score
                        score_breakdown['license'] = license_score

            # 8. Similar popularity tier
            if scoring_config.get('popularity', {}).get('enabled', True):
                if app.stars and target_app.stars:
                    ...
                    if target_tier == app_tier:
                        popularity_score = scoring_config.get('popularity', {}).get('same_tier_score', 1)
                        score += popularity_score
                        score_breakdown['popularity'] = popularity_score

            # 9. Both have or don't have 3rd party dependencies
            if scoring_config.get('dependencies', {}).get('enabled', True):
                if app.depends_3rdparty == target_app.depends_3rdparty:
                    dep_score = scoring_config.get('dependencies', {}).get('same_status_score', 1)
                    score += dep_score
    ...
    def _calculate_semantic_similarity(self, target_app: Application, app: Application, all_applications: List[Application] = None) -> int:
            ...
            for phrase in common_phrases:
                ...
                phrase_weight = self._calculate_phrase_weight(phrase, all_applications)
                score += phrase_weight

            return min(int(score), 25)  # Cap at max_score

    def _calculate_phrase_weight(self, phrase: str, all_applications: List[Application] = None) -> float:
        ...
        common_buzzwords = ['best', 'new', 'free', 'powerful', 'easy', 'simple', 'fast', 'secure', 'reliable', 'free', 'open-source', 'self-hosted','lightweight', 'high-performance']
    ...
    def _is_generic_word(self, word: str) -> bool:
        ...
        'that', 'which', 'who', 'when', 'where', 'why', 'how', 'all', 'any', 'both',
        'each', 'few', 'more', 'most', 'other', 'some', 'such', 'only', 'own', 'same',
        # Common adverbs and adjectives
        'so', 'than', 'too', 'very', 'just', 'now', 'also', 'different', 'small', 'large',
        'new', 'old', 'good', 'great', 'first', 'last', 'long', 'little', 'right', 'big',
        'high', 'following', 'local', 'sure',
        # Common action words that don't indicate domain
        'using', 'used', 'use', 'like', 'way', 'make', 'get', 'go', 'know', 'take', 'see',
        'come', 'think', 'look', 'want', 'give', 'without', 'including', 'provides',
        'allows', 'supports',
        # Common adverisment words
        'best', 'new', 'free', 'powerful', 'easy', 'simple', 'fast', 'secure', 'reliable',
        'free', 'open-source', 'self-hosted', 'community', 'enterprise',
    ...
    def _is_significant_phrase(self, phrase: str, length: int) -> bool:
        ...
        if length == 1:
            return (len(phrase) > 3 and
                    not phrase.isdigit() and
                    not self._is_generic_word(phrase))
    ...
    def _calculate_alternative_similarity(self, target_app: Application, app: Application, scoring_config: Dict) -> int:
        ...
        if (hasattr(target_app, 'alternative_to') and hasattr(app, 'alternative_to') and
            target_app.alternative_to and app.alternative_to):
    ...
    def _calculate_fork_similarity(self, target_app: Application, app: Application, scoring_config: Dict) -> int:
        ...
        if (hasattr(target_app, 'fork_of') and hasattr(app, 'fork_of') and
            target_app.fork_of and app.fork_of):
    ...
    def _minify_html_if_enabled(self, html_content: str) -> str:
        ...
        if self.config.get('generation.minify_html', False):
            if MINIFY_AVAILABLE:
                ...
                print("Warning: HTML minification enabled but minify-html library not available")
                return html_content
        return html_content
